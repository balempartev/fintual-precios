...
IEX es una bolsa. Sus operaciones, barras y cotizaciones llevan hora propia; el resultado del job no hace que todos los símbolos sean recientes. El indicador `volume_activity_proxy_iex` compara volumen IEX acumulado con el día anterior y tiempo transcurrido; no es RVOL de 20 días ni volumen consolidado. Los once ETF sectoriales son referencias, no una taxonomía de todas las acciones. Nunca publicar ranking, porcentajes, spreads ni volumen por símbolo derivados de la API en un repositorio público.
