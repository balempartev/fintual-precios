...
3. `vigilancia.yml`, en un job separado, observa la edad del último barrido, conserva un registro y limita a dos intentos de recuperación por día. Sus incidentes GitHub se asignan al propietario y sus avisos del formulario se deduplican por fecha y hora de Chile. Una falla produce como máximo un email de fallo de workflow por día, conservando el estado técnico en JSON. Sigue dependiendo de GitHub: no vigila los informes de ChatGPT.
4. Las cinco tareas existentes preparan nueve informes laborables y dos entregas dominicales. Consultan la auditoría pública de GitHub, fuentes independientes y, cuando Sites está disponible dentro de la tarea, la última confirmación privada del formulario. Envían comprobantes sin datos financieros a la cuenta propia mediante Gmail, con clave única y búsqueda previa. Cruzan la clave anterior para detectar recibos ausentes y solo recuperan publicaciones omitidas si pueden verificarlas. Un comprobante no prueba publicación ni push. El plan de ChatGPT rechazó crear dos Tasks adicionales por límite de cinco.
...
| Tasks + Gmail + Sites | Tareas, correo propio y formulario ya conectados; dos comprobantes automáticos observados el 24/09 | Sin recibo externo de publicación/push de cada Task ni lectura privada de precios IEX en tarea; límite de cinco Tasks impide recordatorios nuevos | Mantener cinco tareas; separar evidencia de ejecución, mensaje, correo y dispositivo |
...
El 24/09 la captura de apertura `schedule` no apareció; el vigilante la marcó `FAILED`. Un disparo **manual** del vigilante recuperó otra captura, creó incidencia y se recibió correo de recuperación. La programación del vigilante tampoco apareció en esa ventana. No existe aún ejecutor alternativo autenticado y validado: Cloudflare necesita conexión y un mecanismo autorizado para portar credenciales y guardar precios legalmente. Conservar GitHub y los informes mientras se mide su cron.
